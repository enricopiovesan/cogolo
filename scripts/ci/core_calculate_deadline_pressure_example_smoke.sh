...
cd "$repo_root"
pkg="examples/core-calculate-deadline-pressure"
...
require_match "$out" "id: core.calculate-deadline-pressure" "id"
echo "==> wasm abi verify"
out="$("${cli[@]}" wasm abi verify "$pkg/artifacts/core-calculate-deadline-pressure.wasm")"; printf '%s\n' "$out"
...
require_match "$out" "package_id: core.calculate-deadline-pressure-agent" "package"
assert_execute() {
...
assert_execute "$pkg/runtime-requests/uc01-due-soon.json" "ok" "UC-01" \
  '"pressure_band": "medium"' \
  '"days_until_due": 3' \
  '"pressure_score": 0.786'
assert_execute "$pkg/runtime-requests/uc02-overdue.json" "ok" "UC-02" \
  '"pressure_band": "overdue"' \
  '"pressure_score": 1.0'
echo "OK: core.calculate-deadline-pressure 1.0.0 E2E smoke passed"
