...
cd "$repo_root"
pkg="examples/core-decide-escalation"
...
require_match "$out" "id: core.decide-escalation" "id"
echo "==> wasm abi verify"
out="$("${cli[@]}" wasm abi verify "$pkg/artifacts/core-decide-escalation.wasm")"; printf '%s\n' "$out"
...
require_match "$out" "package_id: core.decide-escalation-agent" "package"
assert_execute() {
...
assert_execute "$pkg/runtime-requests/uc01-digest.json" "ok" "UC-01" \
  '"decision": "digest"' \
  '"signals_met": 1'
...
echo "OK: core.decide-escalation 1.1.0 E2E smoke passed"
