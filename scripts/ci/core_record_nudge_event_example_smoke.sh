...
cd "$repo_root"
pkg="examples/core-record-nudge-event"
...
require_match "$out" "id: core.record-nudge-event" "id"
echo "==> wasm abi verify"
out="$("${cli[@]}" wasm abi verify "$pkg/artifacts/core-record-nudge-event.wasm")"; printf '%s\n' "$out"
...
require_match "$out" "package_id: core.record-nudge-event-agent" "package"
assert_execute() {
...
assert_execute "$pkg/runtime-requests/uc01-soft.json" "ok" "UC-01" \
  '"event_id": "nudge-ai-1-1"' \
  '"nudge_ordinal": 1'
assert_execute "$pkg/runtime-requests/uc02-escalate.json" "ok" "UC-02" \
  '"event_id": "nudge-ai-9-4"' \
  '"intensity": "escalate"'
echo "OK: core.record-nudge-event 1.0.0 E2E smoke passed"
