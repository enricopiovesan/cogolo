...
grep -q '"title": "Review Traverse starter app registration"' <<<"$execute_output"
grep -q '"noteType": "project"' <<<"$execute_output"
grep -q '"suggestedNextAction": "expand"' <<<"$execute_output"
grep -q '"status": "complete"' <<<"$execute_output"
