...
grep -q "known_compositions: meeting-notes.process@1.0.0" <<<"$inspect_output"
...
grep -q 'Kickoff notes for Traverse reference app.' <<<"$execute_output"
grep -q '"action_items"' <<<"$execute_output"
grep -q '"decisions"' <<<"$execute_output"
grep -q '"follow_ups"' <<<"$execute_output"
