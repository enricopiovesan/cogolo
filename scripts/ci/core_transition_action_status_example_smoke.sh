#!/usr/bin/env bash
# End-to-end smoke for examples/core-transition-action-status (core.transition-action-status@1.3.0).
...
require_match "$contract_out" "version: 1.3.0" "contract inspect version"
require_match "$contract_out" "service_type: subscribable" "contract inspect service_type"
...
require_match "$pkg_out" "capability_version: 1.3.0" "capability_version"
...
assert_execute() {
  ...
  require_match "$out" "capability_version: 1.3.0" "$label capability_version"
...
echo "==> real emit_event via WasmExecutor"
cargo test -q -p traverse-runtime --test executor_tests core_transition_action_status_

echo "OK: core.transition-action-status 1.3.0 E2E smoke passed"
