...
const MAX_STAKEHOLDERS: usize = 16;

unsafe fn evaluate(input: &[u8], out: &mut [u8]) -> usize {
    let item = object_after_key_at_depth(input, b"\"item\"", 1);
    let stakeholders = array_after_key_at_depth(input, b"\"stakeholders\"", 1);
    let event_type = extract_string_at_depth(input, b"\"event_type\"", 1);
    let config = object_after_key_at_depth(input, b"\"notify_config\"", 1);
    if item.is_none() || stakeholders.is_none() || event_type.is_empty() || config.is_none() {
        return fail(out, b"invalid_input");
    }
    if !matches!(event_type, b"completed" | b"status_changed" | b"blocked") {
        return fail(out, b"invalid_event_type");
    }
    let item = item.unwrap_or(b"{}");
    let stakeholders = stakeholders.unwrap_or(b"[]");
    let config = config.unwrap_or(b"{}");
    let include_mgr = extract_bool(config, b"\"include_manager_on_complete\"").unwrap_or(true);
    let title = extract_string(item, b"\"title\"");
    let owner_name = extract_string(item, b"\"owner_name\"");
    let owner: &[u8] = if owner_name.is_empty() {
        b"Someone"
    } else {
        owner_name
    };

    let verb: &[u8] = match event_type {
        b"completed" => b"marked" as &[u8],
        b"blocked" => b"blocked" as &[u8],
        _ => b"updated" as &[u8],
    };
    let subject_prefix: &[u8] = match event_type {
        b"completed" => b"Action completed: " as &[u8],
        b"blocked" => b"Action blocked: " as &[u8],
        _ => b"Action updated: " as &[u8],
    };

    // First pass: count intents we will emit
    let mut intents = 0u32;
    let mut pos = 1usize;
    while pos < stakeholders.len() && intents < MAX_STAKEHOLDERS as u32 {
        pos += skip_ws_comma(&stakeholders[pos..]);
        if pos >= stakeholders.len() || stakeholders[pos] == b']' {
            break;
        }
        if stakeholders[pos] != b'{' {
            break;
        }
        let Some(end) = balanced_end(&stakeholders[pos..], b'{', b'}') else {
            break;
        };
        let obj = &stakeholders[pos..=pos + end];
        pos += end + 1;
        let user_id = extract_string(obj, b"\"user_id\"");
        if user_id.is_empty() {
            continue;
        }
        let role = extract_string(obj, b"\"role\"");
        if event_type == b"completed" && role == b"manager" && !include_mgr {
            continue;
        }
        intents += 1;
    }
    ...
    if intents == 0 {
        let mut i = 0usize;
        i = copy(out, i, b"{\"intents\":[],\"intent_count\":0,\"reason_code\":\"nothing_to_notify\",\"evaluation_trace\":[\"event_type=");
        i = copy(out, i, event_type);
        i = copy(out, i, b"\",\"prepared 0 intents\"]}");
        return i;
    }

    let mut i = 0usize;
    i = copy(out, i, b"{\"intents\":[");
    let mut wrote = 0u32;
    pos = 1usize;
    while pos < stakeholders.len() && wrote < MAX_STAKEHOLDERS as u32 {
        pos += skip_ws_comma(&stakeholders[pos..]);
        if pos >= stakeholders.len() || stakeholders[pos] == b']' {
            break;
        }
        ...
        i = copy(out, i, b"{\"recipient_id\":\"");
        i = copy_json_escaped(out, i, user_id);
        i = copy(out, i, b"\",\"channel\":\"");
        i = copy_json_escaped(out, i, chan);
        i = copy(out, i, b"\",\"subject\":\"");
        i = copy(out, i, subject_prefix);
        i = copy_json_escaped(out, i, title);
        i = copy(out, i, b"\",\"body\":\"");
        i = copy_json_escaped(out, i, owner);
        i = copy(out, i, b" ");
        i = copy(out, i, verb);
        // Quotes inside the JSON string value must be escaped.
        i = copy(out, i, b" \\\"");
        i = copy_json_escaped(out, i, title);
        if event_type == b"completed" {
            i = copy(out, i, b"\\\" as done.");
        } else {
            i = copy(out, i, b"\\\".");
        }
        i = copy(out, i, b"\"}");
