# ADR-0018: Govern the Local DataStore as an Embedder-Owned Durable Surface

- Status: Accepted
- Date: 2026-07-21
- Governing spec: `518-durable-local-datastore`
...
New local records use a versioned integrity envelope containing the generic versioned key/value record, explicit `public` or `private` classification metadata, and a SHA-256 digest of its canonical serialized content. Reads verify the format, structure, and digest before returning a value. Missing, unknown, malformed, or mismatched integrity information produces the stable `integrity_check_failed` failure with a machine-readable reason. Classification informs later policy work; it does not provide encryption in this decision.

Writes use a temporary sibling and an atomic same-directory commit only after the temporary content has been durably flushed, followed by a parent-directory flush before success is reported. A failed write leaves the prior committed record visible; temporary files are never listed as state. Legacy plain record files are rejected as `legacy_unverified` and must be recreated from an authoritative application source. The adapter holds an exclusive root lock for its owning process; another process receives `store_locked` rather than racing the committed state. The public DataStore operations, capability state schema, Lamport clock, and merge semantics remain unchanged.
...
- SQLite, browser, cloud, replication, multi-process coordination, retention, backup/restore, encryption, and runtime-default storage remain separate decisions; this ADR does not imply their delivery.
